 ...
 */
function increaseItemDepth(opts, transform, listType) {
...
function addAsSubItem(opts, transform, item, destKey) {
    ...
    // An existing list as last child
    ...
    if (existingList) {
        ...
        // Append
        transform = transform.setNodeByKey(destination, {
            nodes: destination.nodes.push(item)
        });

        // and wrap
        const range = transform.state.selection.moveToRangeOf(item);
        const listType = isList(destination) ? destination.type : opts.typeUL;
        return transform.wrapBlockAtRange(range, listType);
    }
}

    ...
    return {
        ...
        normalize(change, node, value) {
            return value.toWrap.reduce(
                (c, { key }) => c.wrapBlockByKey(key, opts.typeItem),
                change
            );
        }
    };
    ...
    return {
        ...
        normalize(change, node, _) {
            ...
            return node.nodes.rest().reduce(
                (c, n, index) => c.moveNodeByKey(n.key, wrapper.key, index + 1, noNorm),
                change
            );
        }
    };
    ...
    return {
        ...
        normalize(change, node, pairs) {
            ...
            pairs.reverse().forEach((pair) => {
                ...
                return change.joinNodeByKey(second.key, first.key, { normalize: false });
            });
        }
    };
}
